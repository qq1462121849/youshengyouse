<template>
  <div id="rightsConter" class="rightsConter" >
    <div class="headerConBox">
      <div class="header">
        <i class="iconfont icon-back" @click="goBack()" v-if="$isShowBackBtn"></i>
        <p class="title">权益中心</p>
        <span class="rightBut" @click="toRecord">我的记录</span>
      </div>
    </div>
      ...
      <mescroll-vue ref="mescroll"  :up="mescrollUp" @init="mescrollInit">
        ...
        <div class="operatorAgency">
          ...
          <div v-if="splendidLList.length > 0" class="splendidBox">
            ...
            <swiper ref="smallImages1" :options="swiperOption2" class="swiper moreSwiper">
              <swiper-slide v-for="(item, index) in splendidLList" :key="'splendid'+index">
                <div  class="iconItem">
        ...
        <div class="upgradeBox" v-if="type == 1 && data">
          ...
          <div class="upOperatorBox" v-if="data.is_show">
            <div v-if="data.is_show.is_user === '1'">
              ...
              <p class="attentionDesc">{{data.requires.note}}</p>
            ...
            <div v-if="data.is_show.is_team === '1'">
              ...
              <p class="attentionDesc">{{data.teamInfo.note}}</p>
              <div class="upBton" @click="goNativeHome('home')">去下单</div>
            </div>
          </div>
        </div>
        <!-- 代理礼包-->
        <div class="shopBox" v-if="type == 0 && agencyBag">
          ...
          <div class="shopItemBox">
            <div class="shopItem" v-if="agencyGoodsList.length > 0" v-for="(item, index) in agencyGoodsList" :key="'shop'+index" @click="jumpTo('/giftBag?id=' + item.id)">
              <div class="imgBox">
            ...
            <div v-if="agencyGoodsList.length === 0" class="haveNoData">
  ...
  methods: {
      ...
      this.$http.get('/amoy/user/partner', {}, true).then((res) => {
        if (res.code === 0) {
          ...
          if (that.agencyBagTab.length > 0) {
            ...
            that.itemID = that.agencyBagTab[0].id
            this.reset()
          }
        }
      })
    },
    upCallback (page, mescroll, itemID) {
      console.log(this.itemID)
      if (this.itemID != '') {
        this.$http.get('/amoy/user/upgrade-goods', {level: this.itemID, page: page.num}, true).then((response) => {
          // 请求的列表数据
          let arr = response.data
          console.log(222)
          // 如果是第一页需手动置空列表
          if (page.num === 1) this.agencyGoodsList = []
          // 把请求到的数据添加到列表
          this.agencyGoodsList = this.agencyGoodsList.concat(arr)
          // 数据渲染成功后,隐藏下拉刷新的状态
          this.$nextTick(() => {
    ...
    resetTab(){
      ...
      if(that.type == 0) {
        ...
        if (that.operatorBagTab.length > 0) {
          that.itemID = that.operatorBagTab[0].id
    ...
    //   that.moneyagencyId = ''
    //   that.agencyGoodsList = ''
    //   console.log('id', that.agencyBagTab[0].id)
    //   if (that.type == 0) {
    //     that.resetTabOne(that.agencyBagTab[0].id)
    //   } else if (that.type == 1) {
    //     that.resetTabOne(that.operatorBagTab[0].id)
