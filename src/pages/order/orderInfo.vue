<template>
  <div id="proof">
    ...
    <div class="content" :style="{marginTop: statusH +0.88+ 'rem'}">
      <div class="topBg">
        <i class="typeIcon"></i>
        <p class="typeTxt">{{orderInfo.status==0?'待付款':orderInfo.status==1?'待收货':'已完成'}}</p>
      </div>
      <div class="oderInfo">
        ...
        <div class="payInfo">
          ...
          <div class="payAuth">
            ...
            <div class="imgcontent">
              ...
              <img class="Img" :src="$store.state.user.userInfo.avatar" alt />
            </div>
          </div>
        </div>
        <div class="payInfo botpayInfo">
          <div class="orderTitle">申诉理由</div>
          <div class="payAuth">
            <p>申诉理由</p>
            <textarea class="textInput" v-model="appealTxt"></textarea>
            <div class="imgcontent">
              <Uploader v-show="!haveImg" class="addImg" :after-read="afterRead"></Uploader>
        ...
        <div class="botomBox">
          <div v-show="appeal!=1" style="color:#FC511F;border-color:#FC511F;">确认收货</div>
          <div v-show="appeal!=1">取消申诉</div>
          <div v-show="appeal==1" class="shensu" @click="toAppeal">申诉</div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  created() {
    this.appeal = this.$route.query.appeal || 0;
  ...
  methods: {
    ...
    toAppeal() {
      if (!this.img || !this.appealTxt) {
        return this.$vux.toast.text("请完善申诉信息！");
      }
      var info = {
        order_id: this.$route.query.id,
        trade_img: this.img,
      ...
      this.$http
        .post("/amoy/trade/trade-complain", info, true, true)
        .then(res => {
          if (res.code == 0) {
