<template>
  <div class="personArticle">
    <div class="articleBox" v-if="list">
      ...
      <div v-if="articles" v-html="articles" class="text-html"></div>
      <div class="box" v-for="(item,index) in items" :key="index" @click="detail(item)">
        <div class="commodity">
          <div class="left">
    ...
    <div v-transfer-dom class="articleBox">
      <popup v-model="show" position="bottom" max-height="50%">
        <goods-item v-bind:list = items @detail="detail"></goods-item>
      </popup>
    </div>
  </div>
</template>
  ...
  methods: {
    get () {
      ...
      this.$http.post('/amoy/hao/talent-article', {id: this.id}, true).then(res => {
        ...
        //       dsBridge.call('openGood', {type: '11', origin_id: id})
