<template>
    <div class="dayingList">
      ...
      <div class="person">
        <div class="avater" :style="{backgroundImage: 'url(' + avatar + ')'}" v-if="avatar" @click="jumpTo('/info')"></div>
        <div class="avater" v-else @click="jumpTo('/info')"></div>
        <div class="info" >
          <p class="sign">{{mobile}}</p>
        </div>
      </div>
      <div class="lists">
  ...
  methods: {
    ...
    getGoods (type) {
      ...
      this.$http.post('/amoy/taobao/goods-item', {
        ...
        if (res.code === 0) {
          let imgNew = this.link + '/amoy/user/create-share?num_iid=' + this.item.goods_id + '&item_url=' + escape(res.data.coupon_url) + '&type=' + res.data.type + '&title=' + encodeURI(res.data.title) + '&thumb=' + res.data.thumb + '&coupon_money=' + res.data.coupon_money + '&coupon_price=' + res.data.coupon_price + '&origin_price=' + res.data.zk_final_price + '&uid=' + this.uid
    ...
    save (j, type) {
      let that = this
      if (type === 3) {
        dsBridge.call('saveMediaToAlbums', that.imgs)
      } else {
        dsBridge.call('systemShare', that.imgs)
      }
    },
    praise (item) {
      var token = dsBridge.call('receiveParams', 'token')
