  ...
  <div id="proof">
    ...
    <div class="content" :style="{marginTop: statusH +0.88+ 'rem'}">
      ...
      <div class="accountBox">
        <div class="copytitle num title">
          <p>支付宝账户：<span>{{config.alipay}}</span> </p>
      ...
      <div class="orderInfo">
        ...
        <div class="copytitle num">
          <p>订单号：<span>{{config.trade_no}}</span> </p>
          <i class="clip" @click="copyTextShare(config.trade_no)"></i>
        </div>
        <div class="copytitle num">
          <p>手机号：<span>{{config.mobile}}</span> </p>
          <i class="clip" @click="copyTextShare(config.mobile)"></i>
        </div>
      ...
      <div class="orderInfo">
        ...
        <div class="imgBox">
          <img v-for="(item, index) in config.img_url" :key="index" :src="item" alt />
        </div>
      </div>
      <div class="box">
          <div class="btn" @click="shensu"  style="background:#666;">提交申诉</div>
...
export default {
  ...
  methods: {
    getConfig() {
      this.$http
        .get(
          "/amoy/trade/confirm-sell-tg-order",
          {
        ...
        .then(res => {
          if (res.code == 0) {
           ...
           let time = parseInt(res.data.period_time);
    ...
    onInput(key) {
      ...
      if (this.pwd.length == 6) {
        ...
        this.$http
          .post(
            "/amoy/trade/confirm-sell-tg-order?order_id=" + this.$route.query.id,
            {
