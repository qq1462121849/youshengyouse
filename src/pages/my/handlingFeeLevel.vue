  ...
  <div id="myLeavel"  :style="{paddingTop:0.88+$store.state.global.statusH+'rem'}">
    ...
    <div class="content">
        ...
        <div class="leavel">
             <div class="lefts">
                 <p>等级</p>
                 <p style="font-size:0.2rem;">手续费</p>
              </div>
              <div class="contain">
    ...
    <div class="upgrade" >
        <p class="bragde">手续费等级</p>
        <ul v-for="(item,index) in arr" :key="index">
            <li>
                <p>手续费等级：{{item.title}}</p>
            </li>
            <li>
                <i >升级条件：</i>{{item.condition}}
            </li>
            <li>
               <p>手续费描述: </p> 
               <div>  
...
export default {
  ...
  methods: {
    async getlevel(){
       let res= await this.$http.post('/amoy/poundage/poundage-level',{},true,true)
       if(res.code==0){
           this.arr=res.data.pound_levels
